...
type PsCompleteCmdlet() =
    ...
    member val CachedRenderer: UI.AnsiRenderer =
        Unchecked.defaultof<UI.AnsiRenderer> with get, set
    ...
    member this.TryExitEarly() =
        if this.CachedHost.FrameHeight < 3 || this.CachedHost.FrameWidth < 1 then
            true
        elif this.CommandCompletion.CompletionMatches.Count = 0 then
            true
                ...
                Render.startLoop
                    ...
                    (fun (state, ctx) ->
                        ...
                        | Input ->
                            ...
                            this.CachedRenderer.RenderCachedState(upd)
                        | Arrow -> this.CachedRenderer.RenderCachedState(state)
                    )

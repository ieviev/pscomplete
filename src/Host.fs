...
type Host(cmdlet: PSCmdlet, bufferString: string, settings: PsCompleteSettings) =
    ...
    let _frameHeight = _initialWindowHeight - _initialCursorY - numOfNewlines - 1
                    ...
                    | :? CommandExpressionAst as ce ->
                        ...
                        | :? MemberExpressionAst as me ->
                            ...
                            | :? TypeExpressionAst as te ->
                                ...
                                let props2 =
                                    rt.GetFields() |> Seq.map (fun v -> v.Name, v.FieldType)

                                Seq.append props1 props2 |> Seq.toArray |> Ok
                            | _ -> Error("no info")
                        | _ -> Error("no info")
                    | _ -> Error("no info")
                else
